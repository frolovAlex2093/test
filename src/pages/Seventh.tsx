...
import { seventhFields } from '../fields/seventhFields';
import {
  brakingType,
  brands,
  categoryId,
...
export const Seventh: React.FC = () => {
  const [blocks, setBlocks] = React.useState(seventhFields);
  ...
  const alertValidation = (pattern: string, value: string): boolean => {
    ...
    if (pattern === '^[0-9]{15}$' && /^[0-9]{15}$/.test(value) === false) return true;
	      if (pattern === '^3(643|112)01[0-9]{9}$' && /^3(643|112)01[0-9]{9}$/.test(value) === false) return true;
        ...
        items.blocksItem.map((item) => {
          ...
          item.blockItem.map((i) => {
            if (
              ...
              if ('mainHidden' in i && i.mainHidden !== undefined && (id === 231 || id === 257)) {
                i.mainHidden[0] = check;
              }
              if (check && (id === 231 || id === 257)) {
                i.hidden = true;
                i.disabled = true;
                i.value[0] = '';
              }
