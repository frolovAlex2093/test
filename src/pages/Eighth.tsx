...
import { eighthFields } from '../fields/eighthFields';
import {
  brakingType,
  brands,
  categoryId,
...
export const Eighth: React.FC = () => {
  const [blocks, setBlocks] = React.useState(eighthFields);
  ...
  const alertValidation = (pattern: string, value: string): boolean => {
	...
	if (pattern === '^3(643|112)01[0-9]{9}$' && /^3(643|112)01[0-9]{9}$/.test(value) === false)
      return true;
        ...
        items.blocksItem.map((item) => {
          ...
          item.blockItem.map((i) => {
            if (
              ...
              if ('mainHidden' in i && i.mainHidden !== undefined && (id === 237 || id === 264)) {
                i.mainHidden[0] = check;
              }
              if (check && (id === 237 || id === 264)) {
                i.hidden = true;
                i.disabled = true;
                i.value[0] = '';
              }
